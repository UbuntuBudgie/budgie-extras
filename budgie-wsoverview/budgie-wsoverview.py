...
from gi.repository import Budgie, GObject, GLib, Gtk
...
modfile = os.path.join(os.environ["HOME"], ".wsindicator_mode")

try:
    mode_index = int(open(modfile).read())
except (FileNotFoundError, ValueError):
    mode_index = 2

modes = ["Simple", "Dotted", "Elipsed dotted", "Menu"]

class WsOverviewWin(object):
    ...
    def change_onthefly(self):
        ...
        open(modfile, "wt").write(str(active))
    ...
    def add_space(self, *args):
        # add one workspace
        new_n = int(wtls.get(["gsettings", "get"] + self.key).strip())+1
        subprocess.Popen(["gsettings", "set"] + self.key + [str(new_n)])
        ...
        while True:
            ...
            if all([self.mode in modes[0:3], t2 == 0]):
                ...
                if self.mode2 != self.mode1:
                    update_menu = True; update_menuset = True
            ...
            if wsdata2 != wsdata1:
                ...
                sp0 = sp[0]; sp1 = sp[1] 
                if sp0 != n_ws:
                    update_menu = True; n_ws = sp0
                if sp1 != curr_ws:
                    set_icon = True; curr_ws = sp1
                wsdata1 = wsdata2

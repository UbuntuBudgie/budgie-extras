...
class BudgieDropByApplet(Budgie.Applet):
    ...
    def setup_dbus_proxy(self):
        """Setup DBus connection to popup window"""
        try:
            self.popup_proxy = Gio.DBusProxy.new_for_bus_sync(
                Gio.BusType.SESSION,
            ...
            # Popup not running yet, that's ok
            self.popup_proxy = None

    def on_destroy(self, *args):
        """Clean up when applet is removed"""
        # Tell popup to close via DBus
        if self.popup_proxy:
            try:
    ...
    def create_windowtrigger(self, *args):
        ...
        if self.popup_proxy:
            try:
                ...
                # Popup may have died, restart it
                self.start_dropover()
                # Try again after a brief delay
                GLib.timeout_add(500, self.retry_show_window)

    def retry_show_window(self):
        """Retry showing window after popup restart"""
        self.setup_dbus_proxy()
        if self.popup_proxy:
            try:
                self.popup_proxy.call_sync(
    ...
    def start_dropover(self):
        ...
        try:
            ...
            # Give it time to start and register on DBus
            GLib.timeout_add(500, self.setup_dbus_proxy)

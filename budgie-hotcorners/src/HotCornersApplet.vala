...
namespace HCSupport {
    ...
    private int get_togglebuttonindex (ToggleButton button, ToggleButton[] arr) {
        for (int i = 0; i < arr.length; i++) {
    ...
    private int get_checkbuttonindex (ToggleButton button, CheckButton[] arr) {
        for (int i = 0; i < arr.length; i++) {
            if (button == arr[i]) return i;
        }
        return -1;
...
namespace HotCornersApplet {
    ...
    public class HotCornersPopover : Budgie.Popover {
        ...
        private void read_checkups (Json.Parser parser, string command) {
